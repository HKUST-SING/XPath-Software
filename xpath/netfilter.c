...
static u32 presto_routing(const struct sk_buff *skb, struct xpath_path_entry *path_ptr);
static u32 ecmp_routing(const struct sk_buff *skb, struct xpath_path_entry *path_ptr);
static u32 rps_routing(const struct sk_buff *skb, struct xpath_path_entry *path_ptr);
...
static u32 ecmp_routing(const struct sk_buff *skb, struct xpath_path_entry *path_ptr)
{
    ...
    hash_key = xpath_flow_hash_crc16(iph->saddr, iph->daddr, (u16)ntohs(tcph->source), (u16)ntohs(tcph->dest));
...
static unsigned int xpath_hook_func_out(const struct nf_hook_ops *ops, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
{
    ...
    if (likely(iph) && iph->protocol == IPPROTO_TCP)
    {
        ...
        if (tcph->syn && unlikely(!xpath_reduce_tcp_mss(skb, sizeof(struct iphdr))))
        {
            if (xpath_enable_debug)
